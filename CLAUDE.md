...
Real-time features use WebSocket connections defined in AsyncAPI contract (`src/types/asyncapi.ts` generated by `bun run generate`). The `useSocket.ts` composable handles WebSocket connections for upload status updates.
...
- Run `bun run typecheck && bun run lint && bun run format` before committing
